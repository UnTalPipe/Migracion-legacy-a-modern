...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Presiona '{FFFFFF}Y{CCCCCC}' para usar el Elevador", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~El piso ya está en la Cola!", 3500, 4);
		else if(DidPlayerRequestElevator(playerid))
		    GameTextForPlayer(playerid, "~r~Tu ya has pedido el Elevador!", 3500, 4);
		else
				...
				if (i <= 12)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
							...
							GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~h~LS BeachSide Elevador ya~n~~y~~h~esta en el Piso...~n~~w~Camina hacia el~n~~w~Y Presiona 'Y'", 3500, 3);

							// Display chat text message to the player
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* El LS BeachSide Elevador ya esta en el Piso... camina hacia dentro de el y presiona '{FFFFFF}Y{CCCCCC}'");
					...
					// Display gametext message to the player
					GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~g~~h~LS BeachSide Elevador~n~~g~~h~Ha sido Llamado...~n~~w~Porfavor Espere!", 3000, 3);
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* El LS BeachSide Elevador ha sido Llamado... se esta moviendo hacia el %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* The LS BeachSide elevator ha sido Llamado... Ahora esta en el %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}Presiona '{FFFFFF}Y{CCCCCC}' Para usar el Elevador", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Presiona '{FFFFFF}Y{CCCCCC}' para Llamar", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "LS BeachSide Elevador...", string, "Accept", "Cancel");
